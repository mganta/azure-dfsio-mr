...
public class TestDFSIOEnh extends Configured implements Tool {
  ...
  protected static void writeTest(FileSystem fs, Configuration fsConfig)
    ...
    fs.delete(DfsioeConfig.getInstance().getDataDir(), true);
    fs.delete(DfsioeConfig.getInstance().getWriteDir(), true);
    ...
    runIOTest(WriteMapperEnh.class, DfsioeConfig.getInstance().getWriteDir(),job);
  }
  ...
  protected static void readTest(FileSystem fs,Configuration fsConfig) throws IOException {
    fs.delete(DfsioeConfig.getInstance().getReadDir(), true);
    ...
    runIOTest(ReadMapperEnh.class, DfsioeConfig.getInstance().getReadDir(),job);
  }
  ...
  protected static void runIOTest( @SuppressWarnings("rawtypes") Class<? extends Mapper> mapperClass,
    ...
    FileInputFormat.setInputPaths(job, DfsioeConfig.getInstance().getControlDir());
  ...
  */
  ...
  private static void createControlFile(
    ...
    fs.delete(DfsioeConfig.getInstance().getControlDir(), true);

    for(int i=0; i < nrFiles; i++) {
      String name = getFileName(i);
      Path controlFile = new Path(DfsioeConfig.getInstance().getControlDir(), "in_file_" + name);
      SequenceFile.Writer writer = null;
      try {
        //writer = SequenceFile.createWriter(fs, fsConfig, controlFile,
         //                                  Text.class, LongWritable.class,
          //                                 CompressionType.NONE);
        writer = SequenceFile.createWriter(fsConfig, SequenceFile.Writer.file(controlFile),
 ...
 protected static String[] calcSummary(final double[] bytesChanged,
    ...
    if (count != 0) {
	 ...
	 protected static void runAnalyse(FileSystem fs, Configuration fsConfig,
		 ...
		 Path reduceFile;
		 if (testType == TEST_TYPE_WRITE)
			 reduceFile = new Path(DfsioeConfig.getInstance().getWriteDir(), "part-00000");
		 else
			 reduceFile = new Path(DfsioeConfig.getInstance().getReadDir(), "part-00000");		
		 ...
		 try {
			 fs.delete(DfsioeConfig.getInstance().getReportDir(), true);
			 //set up env
			 ...
			 job.setMapperClass(Analyzer._Mapper.class);
			 job.setReducerClass(Analyzer._Reducer.class);
			 job.setOutputKeyClass(Text.class);
			 job.setOutputValueClass(Text.class);
//			 job.setNumReduceTasks(1);
			 org.apache.hadoop.mapreduce.lib.input.FileInputFormat.addInputPath(job, reduceFile);
			 org.apache.hadoop.mapreduce.lib.output.FileOutputFormat.setOutputPath(job, DfsioeConfig.getInstance().getReportDir());
			 ...
			 fs.delete(DfsioeConfig.getInstance().getReportTmp(), true);
			 FileUtil.copyMerge(fs, DfsioeConfig.getInstance().getReportDir(), fs, DfsioeConfig.getInstance().getReportTmp(), false, fsConfig, null);
			 LOG.info("remote report file " + DfsioeConfig.getInstance().getReportTmp() + " merged.");
			 BufferedReader lines = new BufferedReader(new InputStreamReader(new DataInputStream(fs.open(DfsioeConfig.getInstance().getReportTmp()))));
			 String line = null;
			 ...
			 lines.close();
			 if (tputReportEach) {
				 FileUtil.copy(fs, DfsioeConfig.getInstance().getReportTmp(), new File(tputResFileName.split(".")[0]+"test_io_.csv"), false, fsConfig);
